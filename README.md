...
| **배운 점** | <ul> <li>실시간 스트리밍 데이터가 <strong>백엔드(FastAPI) → WebSocket → Web UI</strong>로 전달되는 전체 흐름을 직접 구축하면서,<br> <strong>End-to-End 데이터 파이프라인 설계 능력</strong>을 체득했습니다.</li> <li>프론트엔드·백엔드·스트리밍 구조가 상호 의존하는 실시간 서비스 특성상,<br> <strong>비동기 처리·상태 동기화·성능 최적화</strong>의 중요성을 깊게 이해하게 되었습니다.</li> <li>REST API + WebSocket 기반의 <strong>양방향 제어 구조</strong>를 구현하며,<br> UI에서 발생한 액션이 서버→디바이스까지 안정적으로 전달되는 <strong>제어 프로세스 설계 경험</strong>을 쌓았습니다.</li> <li>deploy.sh / systemd 기반의 자동화를 구축하면서,<br> <strong>배포 파이프라인 최적화</strong>와 <strong>운영 효율성</strong>을 높이는 실전 DevOps 경험을 쌓았습니다.</li> <li>실시간 데이터를 Chart.js 기반 Web UI로 시각화하면서,<br> <strong>프론트엔드 퍼포먼스 최적화와 실시간 렌더링 흐름</strong>을 이해했습니다.</li> </ul> |
