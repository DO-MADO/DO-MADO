...
| **목표** | ① <strong>실시간 데이터 스트리밍 파이프라인 구축(Device → FastAPI 서버 → WebSocket → Web UI)</strong> <br>② <strong>브라우저 실시간 시각화 및 데이터 다운로드 기능 제공(Chart.js 기반 실시간 그래프, CSV 저장)</strong> <br>③ <strong>웹 UI에서 변경된 파라미터가 즉시 장비에 반영되는 양방향 제어 API 구축</strong> ④<strong>운영·배포 자동화(원클릭 deploy, systemd 자동 실행) (환경 변수 분리, 배포 스크립트)</strong> |
