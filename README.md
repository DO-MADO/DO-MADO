...
| | |
|---|---|
| **요약** | 센서 기반 실시간 데이터 스트림을 <strong>FastAPI WebSocket</strong> 구조로 가공·전송하고,<br>웹 UI에서 즉시 시각화·저장까지 가능하도록 구현한 <strong>End-to-End 실시간 데이터 처리</strong> 플랫폼입니다. <br><br> 데이터 수집(C 기반 처리부)은 별도의 장치에서 실행되고,<br> FastAPI 서버가 이를 받아 WebSocket → Web UI 로 실시간 전달하는 형태로 구성하였습니다. |
| **목표** | ① <strong>실시간 데이터 스트리밍 파이프라인 구축</strong> (Device → FastAPI 서버 → WebSocket → Web UI) <br>② <strong>브라우저 실시간 시각화 및 데이터 다운로드 기능 제공</strong> (Chart.js 기반 실시간 그래프, CSV 저장)</strong> <br>③ <strong>웹 UI에서 변경된 파라미터가 즉시 장비에 반영되는 양방향 제어 API 구축</strong> <br> ④ <strong>운영·배포 자동화</strong> (원클릭 deploy, systemd 자동 실행 / 환경 변수 분리, 배포 스크립트) |
| **주요 역할** | <ul><li>🧩 <strong>실시간 스트리밍 서버 구축</strong>: FastAPI + WebSocket 기반 실시간 데이터 송수신 구조 개발</li><li>🛠 <strong>양방향 제어 API 구현</strong>: 웹 UI 설정이 서버→장비로 즉시 반영되는 제어 API 구축</li><li>🔁 <strong>End-to-End 제어 프로세스 설계</strong>: UI–서버–장비 간 파라미터 동기화 흐름 설계</li><li>⚙️ <strong>운영·배포 자동화 구축</strong>: deploy.sh 원클릭 배포 + systemd 자동 실행 환경 구성</li><li>📊 <strong>실시간 시각화 UI 개발</strong>: Chart.js 기반 실시간 그래프·CSV 다운로드 제공</li></ul> |
| **Tech Stack** | ![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white) ![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black) ![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white) ![WebSocket](https://img.shields.io/badge/WebSocket-010101?style=for-the-badge&logo=socketdotio&logoColor=white) ![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white) ![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white) ![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white) ![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white) ![Chart.js](https://img.shields.io/badge/Chart.js-FF6384?style=for-the-badge&logo=chartdotjs&logoColor=white) ![systemd](https://img.shields.io/badge/systemd-009639?style=for-the-badge&logo=linux&logoColor=white) ![OpenSSH](https://img.shields.io/badge/OpenSSH-2C2D72?style=for-the-badge&logo=openssh&logoColor=white) ![C](https://img.shields.io/badge/C-A8B9CC?style=for-the-badge&logo=c&logoColor=white) ![ZedBoard](https://img.shields.io/badge/ZedBoard%20(Zynq--7000)-E01F27?style=for-the-badge&logo=xilinx&logoColor=white) |
| **배운 점** | <ul> <li>실시간 스트리밍 데이터가 <strong>백엔드(FastAPI) → WebSocket → Web UI</strong>로 전달되는 전체 흐름을 직접 구축하면서,<br> <strong>End-to-End 데이터 파이프라인 설계 능력</strong>을 체득했습니다.</li> <br> <li>실시간 스트리밍 서비스에서 발생하는 <strong>비동기 처리·성능 최적화 문제</strong>를 직접 해결하며 단단한 실전 감각을 쌓았습니다.</li> <br> <li>REST API + WebSocket 기반의 <strong>양방향 제어 구조</strong>를 구현하며,<br> UI에서 발생한 액션이 서버→디바이스까지 안정적으로 전달되는 <strong>제어 프로세스 설계 경험</strong>을 쌓았습니다.</li> <br> <li>deploy.sh / systemd 기반의 자동화를 구축하면서,<br> <strong>배포 파이프라인 최적화</strong>와 <strong>운영 효율성</strong>을 높이는 실전 DevOps 경험을 쌓았습니다.</li> <br> <li>실시간 데이터를 Chart.js 기반 Web UI로 시각화하면서,<br> <strong>프론트엔드 퍼포먼스 최적화와 실시간 렌더링 흐름</strong>을 이해했습니다.</li> </ul> |
...
| **목표** | ① 복잡한 스파게티 코드를 분석해 <strong>결제 로직을 재설계</strong>하고,<br>② <strong>데스크톱/모바일 UI를 통합</strong>하여 사용자 경험을 개선하며,<br>③ Git이 없는 환경에서도 <strong>안전한 백업·롤백 체계</strong>를 구축하는 것이 목표였습니다. |
...
| **목표** | ① 실사용 가능한 <strong>추천 API</strong>를 직접 설계·구현하고,<br>② <strong>React 프론트엔드 ↔ Express/Flask 백엔드 ↔ Oracle DB</strong>로 이어지는 풀스택 데이터 흐름을 이해·구현하는 것을 목표로 했습니다. |
