...
| | |
|---|---|
| **요약** | 센서 기반 실시간 데이터 스트림을 FastAPI WebSocket 구조로 가공·전송하고,<br>웹 UI에서 즉시 시각화·저장까지 가능하도록 구현한 End-to-End 실시간 데이터 처리 플랫폼입니다. <br><br> 데이터 수집(C 기반 처리부)은 별도의 장치에서 실행되고,<br> FastAPI 서버가 이를 받아 WebSocket → Web UI 로 실시간 전달하는 형태로 구성하였습니다. |
